...
The Data Specification on Mineral Resources – Technical Guidelines (version 3.0) and the associated GML application schema specifying requirements for the interoperability of spatial data sets of the data theme Mineral Resources.
...
5. The IR conformance classes for application schemas without non-abstract feature types, e.g. "Mineral Resources - Core", are not needed as no feature instance can be of a type from that application schema.
...
Note that the TG conformance class is "informative" in the data specification. However, there is no such thing as an informative requirement. For a Technical Guidance, which the data specifications are, the TG requirements are normative and mr-core should be the TG conformance classes.
